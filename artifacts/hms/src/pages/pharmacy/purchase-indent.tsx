...
async function fetchIndents(status?: string) {
  ...
  const r = await fetch(`/api/pharmacy/purchase-indents${p}`);
...
export default function PurchaseIndentPage() {
  ...
  const createIndent = useMutation({
    mutationFn: async (d: any) => { const r = await fetch("/api/pharmacy/purchase-indents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }); if (!r.ok) throw new Error("Failed"); return r.json(); },
  ...
  const approveIndent = useMutation({
    mutationFn: async ({ id, approvedQtys }: any) => { const r = await fetch(`/api/pharmacy/purchase-indents/${id}/approve`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ approvedQtys }) }); if (!r.ok) throw new Error("Failed"); return r.json(); },
  ...
  const rejectIndent = useMutation({
    mutationFn: async ({ id, rejectionReason }: any) => { const r = await fetch(`/api/pharmacy/purchase-indents/${id}/reject`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rejectionReason }) }); if (!r.ok) throw new Error("Failed"); return r.json(); },
  ...
  const pendingCount = (indents as any[]).filter((i: any) => i.status === "pending").length;
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Loading…</TableCell></TableRow>
                    : (indents as any[]).length === 0 ? <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground"><Truck className="w-8 h-8 mx-auto mb-2 opacity-30" /><p>No indents found</p></TableCell></TableRow>
                    : (indents as any[]).map((ind: any) => (
                      <TableRow key={ind.id}>
