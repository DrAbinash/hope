...
import { Plus, Pencil, Search, Share2 } from "lucide-react";

export default function ReferralDoctorsList() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2"><Share2 className="w-6 h-6"/> Referral Doctors</h1>
          <p className="text-muted-foreground text-sm">External referring practitioners and their payment terms.</p>
        </div>
        <Link href="/referrals/doctors/new"><Button><Plus className="w-4 h-4 mr-1"/>Add Referral Doctor</Button></Link>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? <TableRow><TableCell colSpan={6} className="text-center py-6 text-muted-foreground">Loading…</TableCell></TableRow>
              : rows.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center py-6 text-muted-foreground">No referral doctors yet.</TableCell></TableRow>
              : rows.map((d) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell className="text-right"><Link href={`/referrals/doctors/${d.id}/edit`}><Button size="sm" variant="ghost"><Pencil className="w-4 h-4"/></Button></Link></TableCell>
                </TableRow>
              ))}
          </TableBody>
        </Table>
