...
export default function ReferralPayouts() {
  ...
  const [form, setForm] = useState<any>({ referralDoctorId: "", patientId: "", invoiceId: "", serviceDate: new Date().toISOString().slice(0, 10), serviceDescription: "", serviceAmount: "" });
  const [pay, setPay] = useState<any>({ paidOn: new Date().toISOString().slice(0, 10), paymentMode: "cash", reference: "", notes: "" });

  const { data: doctors } = useQuery({
    queryKey: ["/api/referral-doctors", "active"],
    queryFn: async () => {
      const r = await fetch("/api/referral-doctors?active=true", { credentials: "include" });
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["/api/referral-payouts", doctorFilter, statusFilter],
    queryFn: async () => {
      const qs = new URLSearchParams();
      if (doctorFilter) qs.set("doctorId", doctorFilter);
      if (statusFilter) qs.set("status", statusFilter);
      const r = await fetch(`/api/referral-payouts?${qs}`, { credentials: "include" });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      return r.json() as Promise<{ payouts: any[]; summary: any }>;
    },
  });
  ...
  const previewShare = (() => {
    ...
    const v = Number(selectedDoctor.paymentValue) || 0;
    return selectedDoctor.paymentType === "fixed" ? +v.toFixed(2) : +((amt * v) / 100).toFixed(2);
  })();

  async function submit() {
    if (!form.referralDoctorId) { toast({ title: "Pick a referral doctor", variant: "destructive" }); return; }
    ...
    try {
      const r = await fetch("/api/referral-payouts", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.error || `HTTP ${r.status}`); }
      qc.invalidateQueries({ queryKey: ["/api/referral-payouts"] });
      toast({ title: "Referral share recorded" });
      setOpen(false);
      setForm({ referralDoctorId: "", patientId: "", invoiceId: "", serviceDate: new Date().toISOString().slice(0, 10), serviceDescription: "", serviceAmount: "" });
    } catch (e: any) { toast({ title: "Save failed", description: e.message, variant: "destructive" }); }
  }
  async function markPaid(p: any) {
    try {
      const r = await fetch(`/api/referral-payouts/${p.id}`, { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "paid", ...pay }) });
      if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.error || `HTTP ${r.status}`); }
      qc.invalidateQueries({ queryKey: ["/api/referral-payouts"] });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2"><Wallet className="w-6 h-6"/> Referral Shares</h1>
          <p className="text-muted-foreground text-sm">Record services billed under each referral doctor and pay out their share.</p>
        </div>
        <div className="flex gap-2">
          <Link href="/referrals"><Button variant="outline"><ArrowLeft className="w-4 h-4 mr-1"/>Back</Button></Link>
          <Button onClick={() => setOpen(true)}><Plus className="w-4 h-4 mr-1"/>Record Share</Button>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        <Card><CardContent className="p-3"><div className="text-xs text-muted-foreground">Records</div><div className="text-xl font-semibold">{summary.count}</div></CardContent></Card>
        <Card><CardContent className="p-3"><div className="text-xs text-muted-foreground">Total Share</div><div className="text-xl font-semibold">₹{Number(summary.total).toLocaleString("en-IN")}</div></CardContent></Card>
      ...
      <Card><CardContent className="p-4">
        ...
        <Table>
          <TableHeader><TableRow>
            <TableHead>Date</TableHead><TableHead>Doctor</TableHead><TableHead>Patient</TableHead>
            <TableHead>Service</TableHead><TableHead className="text-right">Service Amt</TableHead>
            <TableHead>Basis</TableHead><TableHead className="text-right">Share</TableHead>
            ...
            {isLoading ? <TableRow><TableCell colSpan={9} className="text-center py-6 text-muted-foreground">Loading…</TableCell></TableRow>
              ...
              : (data?.payouts || []).map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell className="text-right font-medium">₹{Number(p.shareAmount).toLocaleString("en-IN")}</TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader><DialogTitle>Record Referral Share</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <div><Label>Service Description</Label><Input value={form.serviceDescription} onChange={(e) => setForm({ ...form, serviceDescription: e.target.value })} placeholder="e.g. OPD consultation, lab tests, surgery"/></div>
            <div><Label>Invoice No / ID (optional)</Label><Input value={form.invoiceId} onChange={(e) => setForm({ ...form, invoiceId: e.target.value.replace(/[^0-9]/g, "") })} placeholder="Invoice ID"/></div>
            {selectedDoctor ? (
              <div className="bg-muted/50 p-3 rounded text-sm">
                <div>Basis: <strong>{selectedDoctor.paymentType === "percentage" ? `${Number(selectedDoctor.paymentValue).toFixed(2)}% of service` : `₹${Number(selectedDoctor.paymentValue).toLocaleString("en-IN")} fixed`}</strong></div>
                <div>Computed share: <strong className="text-emerald-700">₹{previewShare.toLocaleString("en-IN")}</strong></div>
              </div>
            ) : null}
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!payOpen} onOpenChange={(o) => { if (!o) setPayOpen(null); }}>
        <DialogContent>
          ...
          {payOpen ? (
            <div className="space-y-3">
              <div className="text-sm">Doctor: <strong>{payOpen.doctorName}</strong> — Share: <strong>₹{Number(payOpen.shareAmount).toLocaleString("en-IN")}</strong></div>
              <div className="grid grid-cols-2 gap-3">
