...
import { opdVisitsTable, patientsTable, doctorsTable, ipdAdmissionsTable, bedsTable, admissionConversionLogTable, prescriptionQueueTable, medicinesTable } from "@workspace/db";
...
router.put("/opd/:id", async (req, res) => {
  try {
    ...
    // Sync to prescription queue for pharmacy
    if (visit.medicines && Array.isArray(visit.medicines) && visit.medicines.length > 0) {
      try {
        const [existing] = await db.select().from(prescriptionQueueTable)
          .where(eq(prescriptionQueueTable.opdVisitId, visit.id))
          .limit(1);

        const [doctor] = await db.select().from(doctorsTable).where(eq(doctorsTable.id, visit.doctorId));

        const enriched = await Promise.all((visit.medicines as any[]).map(async (item: any) => {
          if (!item.medicineId) return { ...item, available: false, stock: 0 };
          const [med] = await db.select({ stock: medicinesTable.stock })
            .from(medicinesTable).where(eq(medicinesTable.id, item.medicineId));
          const isStopped = !!item.stopped;
          return {
            ...item,
            available: isStopped ? false : (med?.stock ?? 0) >= (item.qty ?? 1),
            stock: med?.stock ?? 0,
            quantity: isStopped ? 0 : item.qty,
            stopped: isStopped
          };
        }));
        const unavailable = enriched.filter((i: any) => !i.available && !i.stopped);

        if (existing) {
          await db.update(prescriptionQueueTable).set({
            prescriptionItems: enriched,
            unavailableItems: unavailable,
            status: visit.status === "completed" ? "pending" : existing.status,
            updatedAt: new Date(),
          }).where(eq(prescriptionQueueTable.id, existing.id));
        } else {
          const queueNo = `RXQ${new Date().toISOString().slice(0, 10).replace(/-/g, "")}${Math.floor(1000 + Math.random() * 9000)}`;
          await db.insert(prescriptionQueueTable).values({
            entityId,
            queueNo,
            patientId: visit.patientId,
            opdVisitId: visit.id,
            doctorId: visit.doctorId,
            doctorName: doctor?.name || "OPD Consultant",
            prescriptionItems: enriched,
            unavailableItems: unavailable,
            priority: "normal",
            status: "pending",
        ...
        req.log.error({ err: errQueue }, "Failed to sync to prescription queue");
      }
    }
