...
export const referralDoctorsTable = pgTable("referral_doctors", {
...
export const insertReferralDoctorSchema = createInsertSchema(referralDoctorsTable).omit({ id: true, createdAt: true });
...
export const referralPayoutsTable = pgTable("referral_payouts", {
  id: serial("id").primaryKey(),
  entityId: integer("entity_id").references(() => entitiesTable.id),
  referralDoctorId: integer("referral_doctor_id").references(() => referralDoctorsTable.id).notNull(),
...
export const insertReferralPayoutSchema = createInsertSchema(referralPayoutsTable).omit({ id: true, createdAt: true });
